...
class DataError(Exception):
    """
    Error class that is raised by invalid data
    """
    pass
...
def create_game_table(conn):
    # create a table
    cursor = conn.cursor() 
    cursor.execute('CREATE TABLE Games ("CreatedDate" timestamp, "FinishDate" timestamp, "GameId" INTEGER PRIMARY KEY, '
                   '"GameLink" TEXT, "TeamA" INTEGER, "TeamB" INTEGER, "Winner" INTEGER, "IsRatingUpdated"  BOOLEAN, '
                   '"IsGameDeleted"  BOOLEAN, "Template" INTEGER)')


def create_leaderboard_table(conn):
    # create a table
    cursor = conn.cursor() 
    cursor.execute('CREATE TABLE Leaderboard ("CreatedDate" timestamp, "Metric" TEXT, "PlayerId" INTEGER, '
                   '"Value" REAL)')
...
def create_clan_leaderboard_table(conn):
    # create a table
    cursor = conn.cursor() 
    cursor.execute('CREATE TABLE ClanLeaderboard ("CreatedDate" timestamp, "ClanName" TEXT, "Metric" TEXT, '
                   '"Value" REAL)')
...
def create_player_table(conn):
    # create a table
    cursor = conn.cursor() 
    cursor.execute('CREATE TABLE Players ("CreatedDate" timestamp, "Name" TEXT, "PlayerId" INTEGER PRIMARY KEY, '
                   '"Rating" INTEGER, "IsRanked" BOOLEAN, "IsJoined" BOOLEAN, "MaxGames" INTEGER, '
                   '"BestRating" INTEGER, "BestRank" INTEGER,"Clan" TEXT,"ActivityBonus" FLOAT, '
                   '"DisplayedRating" INTEGER, "BestDisplayedRating" INTEGER, "WaitCycles" INTEGER, "Rank" INTEGER)')
...
def create_history_table(conn):
    # create a table
    cursor = conn.cursor() 
    cursor.execute('CREATE TABLE  IF NOT EXISTS "History" ("RecordedDate" timestamp NOT NULL, '
                   '"PlayerId" INTEGER NOT NULL, "Rank" INTEGER, "Rating" INTEGER)')


def create_stat_history_table(conn):
    # create a table
    cursor = conn.cursor() 
    cursor.execute('CREATE TABLE IF NOT EXISTS "StatHistory" ("RecordedDate" timestamp NOT NULL, '
                   '"MetricName" TEXT NOT NULL, "Value" FLOAT)')


def insert_game(conn, game):
    cursor = conn.cursor() 
    args = (game.created_date, game.finish_date, game.game_id, game.game_link, game.team_a, game.team_b, game.winner,
            game.is_rating_updated, game.is_game_deleted, game.template)
...
def insert_notable_game(conn, notable_game_record):
    if len(notable_game_record) != 3:
        raise DataError("Invalid notable game record")
...
def insert_veto(conn, veto_record):
    if len(veto_record) != 3:
        raise DataError("Invalid veto record")
...
def insert_or_update_clan(conn, clan_record):
    if len(clan_record) != 3:
        raise DataError("Invalid clan record")
...
def insert_history_record(conn, history_record):
    ...
    if len(history_record) != 4:
        raise DataError("Invalid history record")
...
def insert_stat_history_record(conn, stat_history_record):
    if len(stat_history_record) != 3:
        raise DataError("Invalid stat history record")
...
def update_player(conn, player: Player) -> None:
    ...
    query = ("UPDATE Players SET 'Name' = ?, 'Rating' = ?, 'IsRanked' = ?, 'IsJoined' = ?, 'MaxGames' = ?, "
             "'BestRating' = ?, 'BestRank' = ?, 'Clan' = ?, 'ActivityBonus' = ?, 'DisplayedRating' = ?, "
             "'BestDisplayedRating' = ?, 'WaitCycles' = ?, 'Rank' = ?, 'OnVacation' = ? WHERE PlayerId=?")
    cursor.execute(query, (player.player_name, player.Rating, player.is_ranked, player.is_joined, player.max_games,
                           player.best_rating, player.best_rank, player.clan, player.activity_bonus,
                           player.displayed_rating, player.best_displayed_rating, player.wait_cycles, player.rank,
                           player.on_vacation, player.player_id))
    conn.commit()
...
def find_notable_games(conn, player_id, number_of_notable_games):
    cursor = conn.cursor() 
    cursor.execute('SELECT DISTINCT CreatedDate, GameId FROM NotableGames WHERE "PlayerId"=? '
                   'ORDER BY CreatedDate DESC LIMIT ?', (player_id, number_of_notable_games))
    game_tuples = cursor.fetchall()

    # Players can make multiple updates in a short period of time. If the latest update had less than N games, we want
    # to avoid picking the last N games.
    # Check for the timestamp on each games in the TOP N(N=5). Any games updated before 2 seconds of the latest one, are
    # considered part of a previous update.
    games = []
...
def find_vetoes(conn, player_id, number_of_vetos):
    cursor = conn.cursor() 
    cursor.execute('SELECT DISTINCT CreatedDate, TemplateId FROM Veto WHERE "PlayerId"=? '
                   'ORDER BY CreatedDate DESC LIMIT ?', (player_id, number_of_vetos))
    template_tuples = cursor.fetchall()

    # Players can make multiple updates in a short period of time. If the latest update had less than N templates, we
    # want to avoid picking the last N templates.
    # Check for the timestamp on each template in the TOP N(N=3). Any templates updated before 5 seconds of the latest
    # one, are considered part of a previous update.
    templates = []
    latest_update_time = None
...
def find_history_records(conn, recorded_date=None, player_id=None):
    ...
    if recorded_date is not None:
        ...
        raise DataError("Trying to find history records but both recorded_date and player_id are None.")
    ...
    if all_game_tuples is not None:
        for game_tuple in all_game_tuples:
            ...
            if game.finish_date is not None:
                finish_date = datetime.strptime(game.finish_date[:game.finish_date.rindex(" ") + 9],
                                                '%Y-%m-%d %H:%M:%S')
                elapsed = datetime.now() - finish_date
                if elapsed.days < clot_config.MIN_DAYS_BETWEEN_REMATCH:
                    all_games.append(game)
            else:
                all_games.append(game)
    else:
...
def find_last_n_games(conn, player_id, n):
    ...
    if player_id is not None:
        cursor.execute("SELECT * FROM Games WHERE TeamA=:player OR TeamB=:player ORDER BY CreatedDate DESC LIMIT :n",
                       {"player": player_id, "n": n})
        game_tuples = cursor.fetchall()
...
def find_all_unexpired_games(conn, player_id=None, current_date=None):
    ...
    cursor = conn.cursor()
    if player_id is not None:
        cursor.execute("SELECT * FROM Games WHERE (TeamA=:player OR TeamB=:player) AND FinishDate NOT NULL "
                       "AND FinishDate <= :current_date ", {"player": player_id, "current_date": current_date})
    else:
        cursor.execute("SELECT * FROM Games WHERE FinishDate NOT NULL AND FinishDate <= :current_date "
                       "ORDER BY CreatedDate ASC", {"current_date": current_date})
...
def parse_unexpired_games(game_tuples, permissible_end_date=None):
    ...
    if game_tuples is not None:
        ...
        permissible_start_date = permissible_end_date - timedelta(days=clot_config.DAYS_BEFORE_GAME_EXPIRY)
        for game_tuple in game_tuples:
            game = Game(GameTuple=game_tuple)

            if game.finish_date is not None:
                finish_date = datetime.strptime(game.finish_date[:game.finish_date.rindex(" ")+9], '%Y-%m-%d %H:%M:%S')
                if permissible_start_date < finish_date < permissible_end_date:
...
def find_games_since_last_update(conn, current_date=None):
    """ Find all games which have finished since the last update. 
        ...
        This is useful for replaying scenarios where you want to run the rating calculation for every day over a time
        period.
    """    
    if current_date is None:
        current_date = datetime.now()
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM Games WHERE IsRatingUpdated=0 AND FinishDate NOT NULL AND FinishDate <= :current_date"
                   " ORDER BY CreatedDate ASC", {"current_date": current_date})
...
def find_all_finished_games_by_clan(conn, clan_name):
    ...
    """
    cursor = conn.cursor()
    cursor.execute("SELECT Template, WinnerClan FROM ClanGameInfo WHERE (ClanA=:clan_name OR ClanB=:clan_name) "
                   "AND FinishDate IS NOT NULL", {"clan_name": clan_name})
    all_game_tuples = cursor.fetchall()
...
def get_report(conn, start_date=None, end_date=None):
    ...
    if start_date is None or type(start_date) is not datetime:
    ...
    cursor = conn.cursor()
    query = ('SELECT Players.Clan, Players.PlayerId, Players.Name, U.CurrentRank, U.PreviousRank,  U.RankDifference,'
             ' U.CurrentRating, U.PreviousRating, U.RatingDifference, U.Wins, U.Losses '
             'FROM ('
             'SELECT HT.PlayerId, HT.CurrentRank, HT.PreviousRank, HT.RankDifference, HT.CurrentRating,'
             ' HT.PreviousRating, HT.RatingDifference, WT.Wins, LT.Losses '
             'FROM ('
             ...
             'WHERE H2.RecordedDate = ? AND H1.RecordedDate = ? AND H1.Rank <= 100'
             ') AS HT '
             'LEFT JOIN ('
             'SELECT Loser AS playerId, count(*) AS Losses '
             'FROM ('
             'SELECT Winner, CASE  WHEN Winner = TeamA THEN TeamB ELSE TeamA END AS Loser '
